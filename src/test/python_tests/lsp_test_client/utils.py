...
def as_uri(path: str) -> str:
    ...
    return normalizecase(pathlib.Path(path).absolute().as_uri())


class SnakemakeFile:
    """Create snakemake file on demand for testing."""

    def __init__(self, contents, root):
        self.contents = contents
        self.basename = "".join(
            choice("abcdefghijklmnopqrstuvwxyz") if i < 8 else ".smk" for i in range(9)
        )
        self.fullpath = os.path.join(root, self.basename)

    def __enter__(self):
        """Creates a python file for  testing."""
        with open(self.fullpath, "w", encoding="utf8") as smk_file:
            smk_file.write(self.contents)
        return self
